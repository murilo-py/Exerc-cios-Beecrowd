...
def reduzir(num):
    '''
    Função que reduz uma sequencia de algarismos.
    '''
    if num == 0:
        return 0
    lista = []
    ...
    while len(num) > 1:
        soma = 0
        num = str(num)
        for ciclo in range(len(num)): #transforma em uma lista
            lista.append(int(num[ciclo]))
        
        for passo in range(len(lista)): #soma os itens da lista
            soma += lista[passo]  
