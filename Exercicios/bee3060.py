...
if valor % parcelas == 0: #Verifica se e possivel fazer uma divisão inteira.
    for v in range(parcelas):
        lista_vp.append(valor//parcelas)

else:
    for v in range(parcelas):
        lista_vp.append(valor//parcelas) #Faz o calculo com a divisão inteira, deconsiderando o resto.

    dif = valor - ((valor//parcelas) * parcelas) #Calcula a diferença do valor total das parcelas com base nas parcelas criadas com a div inteira.

    for n in range(len(lista_vp)): #Considerando números inteiros adiciona 1 a cada valor e subtrai 1 da diferença.
        lista_vp[n] += 1
        dif -= 1
        if dif == 0:
            break

for i in range(len(lista_vp)): #Por fim printa o valor das parcelas.
    print(lista_vp[i])